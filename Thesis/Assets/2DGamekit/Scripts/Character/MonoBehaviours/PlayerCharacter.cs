...
namespace Gamekit2D
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        { 
            ...
            if (PlayerInput.Instance.Pause.Down)
            {
                Thesis.GameCore.Log.Write("Input,Pause");
            }

            if (PlayerInput.Instance.Jump.Down)
            {
                Thesis.GameCore.Log.Write("Input,Jump");
            }

            if (PlayerInput.Instance.MeleeAttack.Down)
            {
                Thesis.GameCore.Log.Write("Input,MeleeAttack(K)");
            }

            if (PlayerInput.Instance.RangedAttack.Down)
            {
                Thesis.GameCore.Log.Write("Input,RangedAttack(O)");
            }
        }
